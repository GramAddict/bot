...
def _iterate_over_followers(
    ...
    while True:
        ...
        if is_myself and scrolled_to_top():
            ...
            if (
                not load_more_button_exists
                and screen_iterated_followers == prev_screen_iterated_followers
            ):
                print(
                    COLOR_OKGREEN
                    + "Iterated exactly the same followers twice, finish."
                    + COLOR_ENDC
                )
                return
