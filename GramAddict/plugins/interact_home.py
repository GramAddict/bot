...
class LikesFromHome(Plugin):
    ...
    def __init__(self):
        super().__init__()
        self.description = (
            "Interact with posts from home feed."
        )
        self.arguments = [
            {
                "arg": "--like-from-home",
    ...
    def run(self, device, configs, storage, sessions, plugin):
        ...
        logger.info(f"Handle {self.source}", extra={"color": f"{Style.BRIGHT}. Scrolling and liking for {self.time_at_home} minutes"})
        ...
        def job():
            logger.info(f"Started liking home at: {self.start_time.strftime('%H:%M:%S')}. Will browse for { self.time_at_home } minutes.")
            ...
            logger.info(f"Finished liking from home at {datetime.now().strftime('%H:%M:%S')}")
    ...
    def handle_home(
        ...
        home_view = TabBarView(device).navigateToHome()
        random_sleep(1, 2)
        ...
        while not self.session_state.check_limit(
                self.args, limit_type=self.session_state.Limit.LIKES) and not datetime.now() - self.start_time >= timedelta(
            minutes=self.time_at_home):

            post_description = PostsViewList(device)._check_if_last_post(post_description)
            ...
            if nr_same_post == nr_same_posts_max:
                ...
                if True:
                    ...
                    if storage.is_user_in_blacklist(username):
                        ...
                        logger.info(f"Already liked it. Skip.")
                    elif ad:
                        logger.info(f"Looks like an ad. Skip.")
                    else:
