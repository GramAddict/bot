...
def print_short_report(source, session_state):
    ...
    logger.info(
        f"Session progress: {total_likes} likes, {total_comments} comments, {total_watched} watched, {total_followed} followed, {total_scraped} users scraped, {interactions} successful interaction(s) for {source}",
        extra={"color": f"{Style.BRIGHT}{Fore.YELLOW}"},
    )
