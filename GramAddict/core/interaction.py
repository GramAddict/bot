...
def can_comment(media_type: MediaType, profile_filter, current_mode) -> bool:
    ...
    if can_comment_job:
        if media_type == MediaType.PHOTO and can_comment_photos:
            ...
            return True
    logger.warning(
        f"Can't comment this {media_type} because filters are: ({can_comment_photos=}, {can_comment_videos=}, {can_comment_carousels=}, can_commment_{current_mode}={can_comment_job}. Check your filters.yml."
    )
    return False
